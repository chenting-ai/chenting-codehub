...
from net_and_data import MyDataset, MyDataset2, BSsequential_net_hu, device, weights_init
...
def pre_trained(judge):
    ...
    if judge == 0:
        model = BSsequential_net_hu(BATCH_LEN, num_well).to(device)
    ...
    for epoch in range(epoch_num, EPOCHS+1):
        temp_weight = model.fc5_3.weight   # 检验网络权重是否变化的初始网络参数
        ...
        for num_rand in range(0, number):
            ...
            for itr, (train_dt, train_lable) in enumerate(train_dataloader):
                ...
                temp_b = torch.sum(model.fc5_3.weight.data)
